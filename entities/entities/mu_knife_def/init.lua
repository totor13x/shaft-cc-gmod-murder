...
ENT.Model =  "models/weapons/tfa_csgo/w_knife_default_ct.mdl" 
ENT.WeaponClass = "weapon_mu_knife_def"
...
function ENT:PhysicsCollide( data, physobj )
	...
	if IsValid(ply) && ply:IsPlayer() then
		...
		if ply:GetRole(0) then
			if GAMEMODE:GetRound() == 1 and #team.GetPlayers(2) > 15 then
				RewardPlayer( self:GetOwner(), 20, "бонус за убийство с брошенного ножа")
			end
		end
		
		self:EmitSound("physics/flesh/flesh_squishy_impact_hard" .. math.random(1, 4) .. ".wav")

		if self:GetOwner():GetNWString("murd_t") == "kama" then
			self:GetOwner():PlayTaunt("PriProhojdenii", false)
		end
