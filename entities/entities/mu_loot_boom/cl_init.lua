...
local	color = Color(math.Rand(0,255),math.Rand(0,255),math.Rand(0,255),255)
...
render.ClearStencil()
		render.SetStencilEnable(true)
			...
			self:SetModelScale(1.2,0) --slightly fuzzy, looks better this way
			self:DrawModel()
			self:SetModelScale(1,0)
			render.SetBlend(1)
			self:SetModelScale(1,0)
...
render.ClearStencil()
		render.SetStencilEnable(true)
			...
			render.SetStencilCompareFunction(STENCILCOMPARISONFUNCTION_EQUAL)
			cam.Start3D2D(pos,ang,1)
				local color = Color(0,math.Rand(210,255),0,255)
				if self:GetNWBool("isActivated") then
