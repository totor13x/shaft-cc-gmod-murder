...
function SWEP:PrimaryAttack()
  ...
  if IsValid(tr.Entity) and tr.Entity:GetClass() == "prop_ragdoll" then
    if owner:IsValid() and not owner:UniqueID() or tr.Entity:GetNWInt("OwnerTimeDeath") - ROUND:GetTimer() > 60 then
    ...
    local ply = player.GetByUniqueID(owner:UniqueID())
...
local spawnPos = self:FindPosition(ragdoll)
  ...
  self:SetStateText("ДЕФИРИЛИ - "..string.upper(ply:Name()))
...
function SWEP:HandleRespawn()
  ...
  ParticleEffectAttach("white_lightning",PATTACH_POINT_FOLLOW, ply ,0)
	...
	RewardPlayer( self.Owner, 10, 'за возрождение '.. ply:GetBystanderName() )
  //SendFullStateUpdate()
