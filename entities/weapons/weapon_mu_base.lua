...
function SWEP:PrimaryAttack()
	...
	if !self:IsIdle() and !(act == ACT_VM_FIDGET) then return end
	if self:GetMaxClip1() > 0 && self:Clip1() <= 0 then
		self:Reload()
		return
	end
	//print(self:Clip1(), self:GetMaxClip1())
	...
	if self.Primary.Sequence then
		...
		if type(sequence) == "table" then
			...
			sequence = sequence[(self.LastSequence or -1) + 1]
		end
		vm:SendViewModelMatchingSequence(vm:LookupSequence(sequence))
	end
	local delay = self.Primary.Delay or vm:SequenceDuration()
