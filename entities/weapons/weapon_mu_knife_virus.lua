...
local function playerSend( from, to, force )
	...
	local directions = { -- Directions to try
		...
		math.NormalizeAngle( yawForward - 180 ), -- Left
		yawForward,
	}
	...
	local i = 1
	t.endpos = to:GetPos() + Angle( 180, directions[ i ], 0 ):Forward() * 47 -- (33 is player width, this is sqrt( 33^2 * 2 ))
	...
	while tr.Hit do -- While it's hitting something, check other angles
		...
		if i > #directions then	 -- No place found
			if force then
				...
				return to:GetPos() + Angle( 180, directions[ 1 ], 0 ):Forward() * 47
			else
				return false
			end
		end

		t.endpos = to:GetPos() + Angle( 180, directions[ i ], 0 ):Forward() * 47
