...
concommand.Add("+menu", function (client, com, args, full)
	if client:GetRole(HEADCRAB) then return end
	if client:GetRole(HEADCRAB_BLACK) then return end
	if client:GetRole(CHICKEN) then RunConsoleCommand("mu_taunt", 'help') return end
	...
	if client:Alive() && client:Team() == 2 then
		...
		if LocalPlayer():GetNWString('selectedTaunt') != '' then
			addElement("Таунт (пш)", "taunt_ps")
		end
		if LocalPlayer():GetNWString("LastLooked") ~= "" and GAMEMODE.LookedFade != nil and GAMEMODE.LookedFade + 2 > CurTime() and GAMEMODE.LastLooked:IsPlayer() and  GAMEMODE.LastLooked:Alive() then
			addElement("Я думаю...", "think")
			addElement("Я вижу...", "see")
		end
...
local fontHeight = draw.GetFontHeight("LiDiRadial")
function GM:DrawRadialMenu()
	if radialOpen then
		...
		for k,ment in pairs(ments) do
			...
			drawTextShadow(ment.TransCode, "LiDiRadial", sx + w * 0.6 * x, sy + h * 0.6 * y - fontHeight / 3,textCol, 1)
